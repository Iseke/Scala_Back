...
class TestBot(manager: ActorRef)extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case TestCreate=>
      manager ! MovieManager.CreateMovie(Movie("1","FastFurious", Director("dir-1","Ted","Cooper"),2019))

    case TestConflict=>
      manager ! MovieManager.CreateMovie(Movie("222","Avengers:Infinite War",Director("dir-2","Joe","Russo"),2018))
      manager ! MovieManager.CreateMovie(Movie("222","Spider-Man",Director("dir-3","Anthony","Russo"),2019))
    ...
    case TestUpdate=>
      manager ! MovieManager.UpdateMovie(Movie("1","Avengers:Final",Director("dir-2","Joe","Russo"),2019))
