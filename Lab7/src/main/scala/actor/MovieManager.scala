...
//          log.warning(s"Could not create a movie with ID: ${movie.id} because it already exists.")
          sender() ! Left(ErrorResponse(404, s"Movie with ID: ${movie.id} already exists."))

        case None =>
          movies = movies + (movie.id -> movie)
//          log.info("Movie with ID: {} created.", movie.id)
          sender() ! Right(SuccessfulResponse(200, s"Movie with ID: ${movie.id} created."))
      }
    ...
    case msg: DeleteMovie=>
      movies.get(msg.id) match {
        case Some(existingMovie)=>
          ...
          sender() ! Right(SuccessfulResponse(200, s"Movie with ID: ${existingMovie.id} deleted."))

        case None =>
         sender() ! Left(ErrorResponse(404, s"Movie with ID: ${msg.id} does not exists."))
      }
  }
